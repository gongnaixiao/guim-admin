...
Ext.define("AM.ux.Action", {
    ...
    doJSONAction: function (obj) {
        ...
        Ext.Msg.show({
            ...
            fn: function (btn) {
                if (btn == 'ok') {
                    var myMask = new Ext.LoadMask(Ext.getBody(), {msg:"正在执行..."});
                    myMask.show();
                    Ext.Ajax.request({
                        url: obj.url,
                        jsonData: Ext.encode(obj.params),
                        success: function (response) {
                            myMask.hide();
                           json = Ext.decode(response.responseText);
                            if (json.message) {
                                Ext.info('操作成功, ' + json.message);
                            } else {
                                Ext.info('操作成功');
                            }
                            if (obj.grid) {
